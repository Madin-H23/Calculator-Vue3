...
> 完整 29 步 commit hash 见 [`docs/ai-chat/index.md`](docs/ai-chat/index.md)。另有 2 个前置规划提交(`03d4181`、`6daca01`)与若干交付后内存扩展提交(单寄存器 → 栈模型 → M▾ 浮窗,详见 `docs/ai-chat/05-memory.md`)。

## 还原说明

- **视觉基准**:`PROMPT.md` §4(色板 / 4×6 网格 / 字号 / 浅深主题)。
- **功能**:`8 + 9 = 17`、`9 ÷ 0 =` → `不能除以零`、`1/x`、`x²`、`√x`、负数开方 → `无效输入`、`%`、`±`、`⌫`、`C / CE`、小数点防重复;**内存** MC/MR/M+/M-/MS(内存栈)+ **`M▾` 浮窗**(栈列表 + 逐项 MC/M+/M- + 空态 + 点击外部关闭)+ `M` 徽标;运算符选中高亮;🌙 / ☀ 浅 / 深主题切换。
